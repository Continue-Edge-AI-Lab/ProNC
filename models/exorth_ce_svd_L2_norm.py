...
import math
import pandas as pd
from utils.L2norm_loss import L2NormLossDetailed
...
class extend_orth(ContinualModel):
    ...
    def __init__(self, backbone, loss, args, transform, dataset=None):
        ...
        self.old_net = None
        self.loss = DRLoss()
        self.distill_loss = L2NormLossDetailed()
    ...
    def observe(self, inputs, labels, not_aug_inputs, logits=None, epoch=None):
        ...
        if self.current_task == 0:
            ...
            self.ce_loss_item += loss.item()

        else:
            self.class_to_idx_map = {class_label.item(): idx for idx, class_label in enumerate(self.classes_so_far)}
            ...
            # if self.current_task > 0:
            #     class_mean = self.class_mean / torch.norm(self.class_mean, p=2, dim=1, keepdim=True)
            #     proto_vec = (epoch/50)*etf_vec + (1-epoch/50)*class_mean
            #     use_proto = proto_vec / torch.norm(proto_vec, p=2, dim=1, keepdim=True)
            # else:
            use_proto = etf_vec
            ...
            loss = self.main_weight * self.distill_loss(F.normalize(outputs, dim=1), use_proto[labels, :].to(self.device))
            self.dr_loss += loss.item()
            ce_loss= self.ce_weight * self.ce_loss(logits, labels)
            self.ce_loss_item += ce_loss.item()
            loss += ce_loss
            self.total_loss += loss.item()

            old_feature = self.old_net(inputs, returnt='features')

            distill_loss = self.distill_weight * self.distill_loss(F.normalize(outputs, dim=1), F.normalize(old_feature, dim=1))
            loss = loss + distill_loss
            self.total_loss += distill_loss.item()
            self.dist_loss += distill_loss.item()
    ...
    def begin_epoch(self, epoch: int, dataset: ContinualDataset) -> None:
        ...
        self.dist_loss = 0.0
        self.total_loss = 0.0
        self.ce_loss_item = 0.0


        # if self.current_task > 0:
        #     buf_inputs, buf_labels = self.buffer.get_all_data()
        #     self.get_class_mean(buf_inputs, buf_labels)

    def end_epoch(self, epoch: int, dataset: ContinualDataset) -> None:
        iter = len(dataset.train_loader)
        current_loss = {"epoch": epoch, "dr_loss": self.dr_loss/iter, "dist_loss": self.dist_loss/iter,
                        "total_loss": self.total_loss/iter, "ce_loss": self.ce_loss_item/iter}

        for key, value in current_loss.items():
            self.loss_df[key].append(value)
    ...
    def begin_task(self, dataset):
        ...
        if self.current_task == 1:
            ...
            self._init_etf()
        self.loss_df["epoch"] = []
        self.loss_df["dr_loss"] = []
        self.loss_df["dist_loss"] = []
        self.loss_df["total_loss"] = []
        self.loss_df["ce_loss"] = []
    ...
    def end_task(self, dataset) -> None:
        ...
        df = pd.DataFrame(self.loss_df)
        script_dir = os.path.dirname(os.path.abspath(__file__))
        parent_dir = os.path.dirname(script_dir)
        folder_path = f"loss/exorth_svd_ce_MMD_{self.base_task}/{self.dataset.NAME}"
        file_name = f"loss_task_{self.current_task}.csv"
    ...
    #     cosine_simi = F.cosine_similarity(self.etf, F.normalize(self.class_mean).T, dim=0)
    #     print(f"Task {self.current_task} cosine similarity between class mean and etf: ")
    #     print(cosine_simi)
    #     print("Mean of the Cosine Similarity: ")
    #     print(cosine_simi.mean())

    #     if self.current_task >= self.base_num/self.dataset.N_CLASSES_PER_TASK:
    #         for i in range(0, self.current_task):
    #               cosine_simi = F.cosine_similarity(self.old_etf[i], self.etf[:, 0:self.old_etf[i].size(1)], dim=0)
    #               print(f"Task {i+1} cosine similarity with current task: ")
    #               print(cosine_simi)
    #               print("Mean of the Cosine Similarity: ")
    #               print(cosine_simi.mean())
    ...
    #     if self.current_task > 0:
    #         for i in range(0, self.current_task):
    #               cosine_simi = F.cosine_similarity(self.all_old_class_mean[i], self.class_mean[0:self.all_old_class_mean[i].size(0),:], dim=1)
    #               print(f"Task {i} cosine similarity with current class mean: ")
    #               print(cosine_simi)
    #               print("Mean of the Cosine Similarity: ")
    #               print(cosine_simi.mean())

    #         if self.current_task >= self.base_num/self.dataset.N_CLASSES_PER_TASK:
    #             cosine_simi = F.cosine_similarity(self.old_etf[-1], self.class_mean[0:self.old_etf[-1].size(1),:].T, dim=0)
    #         elif self.current_task > 0:
    #             cosine_simi = F.cosine_similarity(self.old_etf[-1][:, 0:self.class_mean.size(0)], self.class_mean.T, dim=0)
    #         print(f"Task {self.current_task+1} cosine similarity between class mean and old etf: ")
    #         print(cosine_simi)
    #         print("Mean of the Cosine Similarity: ")
    #         print(cosine_simi.mean())

    #     self.old_etf.append(self.etf)
    #     self.all_old_class_mean.append(F.normalize(self.class_mean))
    #     #self.print_test = True

    # # def get_global_mean(self, inputs):
